...
const geminiResponse = async (command, assistantName, userName) => {
  try {
    ...
    if (!apiKey) {
      throw new Error("❌ GEMINI_API is missing in .env file");
    }

    // ✅ Use API URL from .env if available, otherwise fallback to 1.5-flash
    const apiUrl = process.env.GEMINI_API_URL 
      ? (`${process.env.GEMINI_API_URL}?key=${apiKey}`)
